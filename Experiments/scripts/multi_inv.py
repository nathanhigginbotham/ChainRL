...
class InvManagementMasterEnv(MultiAgentEnv):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.num_agents = 3  # hard coded for now
        self.truncateds = set()
        ...
        self.action_space = gym.spaces.Dict(
            {
                "0": gym.spaces.Box(
                    low=0, high=self.supply_capacity[0], dtype=np.int32
                ),
                "1": gym.spaces.Box(
                    low=0, high=self.supply_capacity[1], dtype=np.int32
                ),
                "2": gym.spaces.Box(
                    low=0, high=self.supply_capacity[2], dtype=np.int32
                ),
            }
        )
        ...
        single_pipeline = lt_max + 1

        self.observation_space = gym.spaces.Dict(
            {
                "0": gym.spaces.Box(
                    low=-np.ones(single_pipeline)
                    * self.supply_capacity.max()
                    * self.num_periods
                    * 10,
                    high=np.ones(single_pipeline)
                    * self.supply_capacity.max()
                    * self.num_periods,
                    dtype=np.int32,
                ),
    ...
    def _RESET(self, *, seed=None, options=None):
        ...
        # set state
        self._update_state()
        obs1 = self.state[0:-2:3]
        obs2 = self.state[1:-1:3]
        obs3 = self.state[2::3]

        ob_list = [obs1, obs2, obs3]
    ...
    def _STEP(self, action_dict):
        ...
        obs, rew, terminated, truncated, info = OrderedDict(), {}, {}, {}, {}
        ...
        p1 = a**n * (p[0] * S[0] - (r[0] * RR[0] + k[0] * U[0] + h[0] * II[0]))
        p2 = a**n * (p[0] * S[1] - (r[0] * RR[1] + k[0] * U[1] + h[0] * II[1]))
        p3 = a**n * (p[0] * S[2] - (r[0] * RR[2] + k[0] * U[2] + h[0] * II[2]))
        ...
        if terminated["0"] == False:
            terminated["__all__"] = 0
        elif terminated["0"] == True:
